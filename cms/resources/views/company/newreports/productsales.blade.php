...
@section('title', 'Product Sales Order Reports')
    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Product Sales Order Report </h3>
          {{-- <p class="notice-info" style="
            color: #cc942f;
        "> *Note: Only approved orders are considered in this report.</p> --}}
        </div>
        <!-- /.box-header -->
        <div class="box-body tablediv">
            ...
            @endif
          <form action="{{domain_route('company.admin.productsalesreports')}}" method="post" id="productsalesreports">
            @csrf
            <div class="row no-margin">
              ...
              <div class="col-xs-3 no-lt-pd">
                <label>
                  Select Order Creator
                </label>
                <select id="salesman_id" name="salesman_ids[]" class="multi" style="width: 100%;" multiple>
                  @foreach($salesman as $key=>$value)
                  <option value="{{$key}}" selected>{{$value}}</option>
                  @endforeach
                </select>
                <span class="salesman_id"></span>
                 @if($errors->has('salesman_ids'))
                  <p class="help-block has-error" style="color:red;">{{$errors->first('salesman_ids')}}</p>
                  @endif
              </div>
              ...
              <div class="col-xs-3 no-lt-pd">
                ...
                <select id="party_id" name="party_ids[]" class="select2" style="width: 100%;" multiple>
              ...
              <div class="col-xs-6 col-xs-offset-3" style="padding-top:20px;">
                ...
                <div class="col-xs-4" style="width:max-content;">
                  <label class="radio inline">
                    <input class="report_type" name="report_type" type="radio" name="dlywkly" value="12" checked>Monthly Report
                  </label>
                </div>
                <div class="col-xs-4" style="width:max-content;">
                  <label class="radio inline">
                    <input class="report_type" name="report_type" type="radio" name="dlywkly" value="30">Daily Report
                  </label>
                </div>
              </div>
            </div>
      ...
      <div class="box" id="mainBox">
        ...
        <div class="box-body">
          ...
          <table id="order" class="table table-bordered table-striped table-responsive" style="width: 100%;">
            ...
            <tbody>
              {{-- @foreach ($reports_generated->toArray() as $key=>$report)
              <tr>
                <td class="hidden"></td>
                <td>{{ $report["created"] }}</td>
                <td>{{ strtoupper($report["report_cat"]) }}<span class='fa fa-info-circle' aria-hidden='true' data-html='true' data-toggle='tooltip' data-original-title="{!!$report["content"]!!}"></span></td>
                <td>{{ $report["date"] }}</td>
                <td>
            ...
            <div id="loader1" hidden>
              ...
              {{-- <p style="margin: 5px 0 0 0; font-weight: bolder;">
...
<script type="text/javascript">
  ...
  $('#productsalesreports').on('submit', function (event) {
      ...
      var salesman_ids = $('#salesman_id').val();
  ...
  function initializeOrderDT(){
    table = $('#order').DataTable({
      ...
      "ajax":{
        "url": "{{ domain_route('company.admin.productsalesreportsdt') }}",
        "dataType": "json",
        "type": "POST",
        "data":{  
          _token: "{{csrf_token()}}", 
