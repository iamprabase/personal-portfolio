    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Party Report</h3>
        </div>
        <!-- /.box-header -->
        <div class="box-body tablediv">
          @if(\Session::has('success'))
            ...
            @endif
          <form action="{{domain_route('company.admin.generatepartydetailsreport')}}" method="post" id="dailysalesmanreport">
            @csrf
            <input type="hidden" id="partyids" name="partyids">
            <input type="hidden" id="colids" name="colids">
            <div class="row hidden" id="maindatarow">
              <div class="col-xs-2">
                <label>
                  Select Party Type
                </label>
                <select id="party_type" style="width: 100%;" class="parties" multiple>
...
<script type="text/javascript">
    ...
    $("#maindatarow").removeClass('hidden');
    ...
    function initializeOrderDT(){
      table = $('#order').DataTable({
        ...
        "ajax":{
          "url": "{{ domain_route('company.admin.generatepartydetailsreportdt') }}",
          "dataType": "json",
          "type": "POST",
          "data":{  
            _token: "{{csrf_token()}}", 
    ...
    $("#getReport").on('click',function(){
      var arr = []; var str = '';
      $("#handled_party_id :selected").each(function(){
        arr.push(this.value);
        str = arr.join(',');
      });
      $('#partyids').val(str);
