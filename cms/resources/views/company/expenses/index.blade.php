...
@section('content')
        ...
        <div class="box">
          <div class="box-header">
            <div class="row">
              ...
              <div class="col-xs-4">
                <span class="pull-right" id="totalEAmount" style="line-height: 3;"></span>
              </div>
              <div class="col-xs-4">
                @if(Auth::user()->can('expense-create'))
                 <a href="{{ domain_route('company.admin.expense.create') }}" class="btn btn-primary pull-right"
  ...
  <form method="post" action="{{domain_route('company.admin.expense.customPdfExport')}}" class="pdf-export-form hidden"
    id="pdf-generate">
    {{csrf_field()}}
...
<script>
    $(document).ready(function () {
                @if (strpos(URL::previous(), domain_route('company.admin.expense')) === false)
        var activeRequestsTable = $('#expense').DataTable();
    ...
    $(function () {
        ...
        @if($nCal==0)
          ...
          $.fn.dataTable.ext.search.push(
              function (settings, data, dataIndex) {
                  ...
                  var create_date = Date.parse(data[1]); // use data for the age column
        ...
        var end = $('#end_edate').val();
        var empVal = null;
        @if (\Session::has('DT_Exp_filters'))
          let filtersSel = @json(\Session::get('DT_Exp_filters'));
          sessionStorage.setItem('DT_Exp_filters', filtersSel);
        @else
          sessionStorage.setItem('DT_Exp_filters', "");
        @endif
        if(sessionStorage.getItem('DT_Exp_filters')!="" || !sessionStorage.getItem('DT_Exp_filters')==undefined ){
          let filterValue = JSON.parse(sessionStorage.getItem('DT_Exp_filters'));
          if(filterValue){
        ...
        function initializeDT(empVal=null, startD, endD){
          ...
          "stateSave": true,
          "ajax":{
             "url": "{{ domain_route('company.admin.expense.ajaxTable') }}",
             "dataType": "json",
             "type": "POST",
             "data":{ 
              _token: "{{csrf_token()}}",
          ...
          {
            $('#totalEAmount').html(settings.json.total);
          }
          });
        ...
        $('#reportrange').on('click',function(){
          if ($(window).width() <= 320) {   
            $(".daterangepicker").addClass("expensedateposition");
            ...
            $(".daterangepicker").removeClass("expensedateposition");
          }
        });
        ...
        $('#expense').on('click', '.alert-modal',function(){
          console.log('clicked');
