...
<section class="content">
  <div class="box box-default">
    ...
    <div class="box-body switch-edit">
      {!! Form::open(array('url' => url(domain_route("company.admin.product.store", ["domain" =>request("subdomain")])),
      'method' => 'post', 'id'=>'formSubmit','files'=> true)) !!}
...
<script>
  ...
  $(document).ready(function () {
    let text_max = 60;
    $('#short_desc_feedback').html(text_max + ' characters remaining');

    $('#short_desc').keyup(function () {
      let text_length = $('#short_desc').val().length;
  ...
  $('#dynamic_field').on('click', '.btn-add', function(e){
    ...
    let nextVariantField = `<input name="variant[${rowId}]" type="text" class="form-control variantClass" id="variant${rowId}" placeholder="Variant..." required>`;
    @if($getClientSetting->var_colors == 1)
      let colorsField = `<select class="form-control colorClass" id="colors${rowId}" data-id="${rowId}" multiple name="var_colors[${rowId}][]">@forelse($colors as $key=>$color)<option value="{{$key}}">{{$color}}</option>@empty<option value="0"></option> @endforelse</select><span class="hidden hiddenText" id="hiddenText${rowId}">Only Variants can have attributes.</span>`;
    @endif

    let mrpField = `<input class="form-control onlynumber" placeholder="Rate" id="mrp${rowId}" data-id="${rowId}" required name="mrp[${rowId}]" type="text" @if(config('settings.order_with_amt')==1) value="0" @endif>`;

    let unitField = `<select class="form-control unitClass" id="unit${rowId}" data-id="${rowId}" name="unit[${rowId}]" required><option></option>@if(!empty($units)) @foreach($units as $id=>$unit)<option value="{{$id}}">{{$unit}}</option>@endforeach @endif</select>`;

    let shortDescField = `<input class="form-control" id="short_desc${rowId}" rows="${rowId}" cols="54" style="resize:none" maxlength="60" placeholder="Short Description" name="short_desc[${rowId}]" type="text">`;
    ...
    @if($getClientSetting->var_colors == 1)
      $('#dynamic_field').append(`<tr class="rowElement" id="rowElement${rowId}" data-row_id="${rowId}"><input type="hidden" name="numofRows[]" value="${rowId}"><td>${nextVariantField}</td><td>${colorsField}</td><td @if(config('settings.order_with_amt')==1) hidden @endif>${mrpField}</td><td>${unitField}</td><td>${shortDescField}</td><td>${actionBtn}</td></tr>`);
    @else
      $('#dynamic_field').append(`<tr class="rowElement" id="rowElement${rowId}" data-row_id="${rowId}"><input type="hidden" name="numofRows[]" value="${rowId}"><td>${nextVariantField}</td><td @if(config('settings.order_with_amt')==1) hidden @endif>${mrpField}</td><td>${unitField}</td><td>${shortDescField}</td><td>${actionBtn}</td></tr>`);
    @endif
    ...
    @if($getClientSetting->var_colors==1)
      ...
      //     $(getId).multiselect('reload');
      //     // if(jQuery.inArray("0", $(getId).val())==0){
      //     //   alert("At least one option in the list must remain checked.");
      //     // }
      //   }

      //   if(getVal.length>1 && jQuery.inArray("0", getVal)==0){
      //     let removedZeroVal = getVal.splice( getVal.indexOf("0"), 1 );
      //     let getId = `#${$(this).attr("id")}`;
      //     $(getId).val(getVal);
      //     $(getId).multiselect('reload');
      //   }
  ...
  $('#varONOFF').on('change',function(e){
    ...
    if(currentElement.hasClass('OFF')){
      ...
      $('.colorClass').next().removeClass("hidden");
