...
@section('title', 'Activities Types')
...
@section('content')
      ...
      <div class="col-xs-12">
        ...
        <div class="box">
          ...
          <div class="box-header">
            ...
            <h3 class="box-title">Activities Types</h3>
  ...
  <div class="modal fade" id="AddActivityType" tabindex="-1" role="dialog">
    <form id="addNewActivityType" method="post" action="{{domain_route('company.admin.activities-type.store')}}">@csrf
  ...
  <div class="modal fade" id="EditActivityType" tabindex="-1" role="dialog">
    <form id="editExistingActivityType" method="post"
          action="{{domain_route('company.admin.activityType.update')}}">@csrf
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                  aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" align="center">Update Activity Type</h4>
          </div>
          <div class="modal-body">
            <div class="row">
  ...
  <div class="modal fade" id="DeleteActivityType" tabindex="-1" role="dialog">
    <form id="deleteExistingActivityType" method="post"
          action="{{domain_route('company.admin.activityType.delete')}}">@csrf
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-xs-12">
                <div align="center">
                  Are you sure you want to Delete Activity Type(<span id="del_title"></span>) ?
                </div>
  ...
  <form method="post" action="{{domain_route('company.admin.activityType.customPdfExport')}}" class="pdf-export-form hidden"
    id="pdf-generate">
    {{csrf_field()}}
  ...
  <script>
    ...
    var table;
      $(function () {
          ...
          $('#employeegroups').on('click','.rowEditActivityType',function () {
              $('#edit_id').val($(this).attr('activity-id'));
              $('#edit_name').val($(this).attr('activity-name'));
              $('#EditActivityType').modal('show');
          });

          $('#employeegroups').on('click','.rowDeleteActivityType', function () {
              ...
              $.ajax({
                  ...
                  success: function (data) {
                      ...
                      for (i = 0; i < data['count']; i++) {
                          var editurl = "{{ domain_route('company.admin.activities-type.edit',['id']) }}";
                          ...
                          table.row.add([
                              ...
                              '<a  class="btn btn-warning btn-sm rowEditActivityType" activity-id="' + data['activityType'][i]['id'] + '" activity-name="' + data['activityType'][i]['name'] + '" style=" padding: 3px 6px;"><i class="fa fa-edit"></i></a>'+data['del_action'][i],
                          ]).draw();
                      }
                  },
              });
          ...
          $('#editExistingActivityType').on('submit', function (event) {
              ...
              var url = "{{domain_route('company.admin.activityType.update')}}";
              $.ajax({
                  headers: {
                      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                  },
          ...
          $('#deleteExistingActivityType').on('submit', function (event) {
              event.preventDefault();
              var edit_id = $('#edit_id').val();
              var url = "{{domain_route('company.admin.activityType.delete')}}";
              $.ajax({
                  headers: {
                      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                  },
