        ...
        <div class="invoice">
          <div class="row no-gutters d-lg-flex">
            ...
            <div class="invoice-right col-md-6 col-xs-10 col-centered">
              ...
              <img src="{{ URL::asset('cms/'.config('settings.logo_path')) }}" alt="{{config('settings.title')}}" style="max-height: 150px;">
              @endif
              <h1>{{ ucfirst(config('settings.title')) }}</h1>
              <h3>Sign In</h3>
              <form action="{{ domain_route('company.login.post') }}" method="post">
                {{ csrf_field() }}
                ...
                <div class="form-group d-flex-custom">
                  <div class="checkbox icheck">
                    <label>
                      <input type="checkbox" name="is_remember"> Stay signed in
                    </label>
                  </div>
                  <div>
                    @if(!in_array(config('settings.company_id'), array(130,20,14)))
                    <a href="{{domain_route('company.showForgotPassword')}}">Forgot Password?</a>
                    @endif
                  </div>
                </div>
