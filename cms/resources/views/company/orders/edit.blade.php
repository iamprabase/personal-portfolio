...
@section('content')
        ...
        <div class="box box-default">
            <div class="box-header with-border">
                ...
                <div class="page-action pull-right">
                    <a href="{{ domain_route('company.admin.order') }}" class="btn btn-default btn-sm"> <i
            ...
            <div class="box-body">
                ...
                {!! Form::model($order, array('url' => url(domain_route('company.admin.order.update',[$order->id])) , 'method' =>
                'PATCH', 'autocomplete' => 'off', 'files'=> true, 'id'=>'orderForm')) !!}
    ...
    <div class="modal modal-default fade" id="deleteorderproducts" tabindex="-1" role="dialog"
            ...
            <div class="modal-content">
                ...
                <form method="post" class="remove-record-model" id="formDeleteOrderProducts">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-warning delete-button" id="deleteConfirm">Yes, Delete
                        </button>
                    </div>
                </form>
            </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        let enablePartyCategoryRate = Boolean({{ $getClientSetting->category_wise_rate_setup}});
        const companyName = "{{Auth::user()->companyName($getClientSetting->company_id)->company_name}}";
        ...
        $(document).ready(function () {
            ...
            $(document).on('click', '.addMoreOrder', function () {
                ...
                if (getCountRow <= 1) {
                    $(`#remove${befIncrement}`).removeClass("hidden");
                }
                @endif
        ...
        tax_types.default_flag" @endphp
                <option value="{{$product->id}}" data-brand="{{isset($product->brand)?$product->brand:null}}" data-category="{{isset($product->category_id)?$product->category_id:null}}"  data-product_name="{{$product->product_name}}"
...
@endforelse`;
        ...
        data-rowNum="${rowNum}" data-orderNo="">X</button><button type="button" name="add" id="add${rowNum}" class="btn btn-success form-control addCancelBtn addMoreOrder" data-rowNum="${rowNum}" data-orderNo="">+</button>`;
          ...
          <input type="hidden" id="hidden_u_id${rowNum}">
            ...
            @if($partyTypeLevel)
            async function setOrderTo(selectedClientType, superior) {
                const parties = @json($clients);
                if ("{{$getClientSetting->order_to}}" == 1) {
                    $('#orderTo').append(` <option value="" ${superior ? "" : "selected"}>${companyName}</option>`)
                }
                await $.ajax({
                    url: "{{ domain_route('company.admin.orderTo') }}",
                    method: "GET",
                    dataType: 'json',
                    data: {client_type: selectedClientType, order_to: "{{$order->order_to}}"},
                    beforeSend: function () {
                        showLoader()
                    },
                    success: function (response) {
                        ...
                        if (Object.entries(data).length !== 0) {
                            const filteredParties = data;//parties.filter(party => data.includes(party.client_type));
                            Object.keys(filteredParties).map((index, key) => {
                                let optionEntry = "{{$getClientSetting->order_to}}" == 1 ? `<optgroup label="${index}">` : "";
                                if ("{{$getClientSetting->order_to}}" == 1) {
                                    filteredParties[index].map((filteredParty, index) => {
                                        optionEntry += ` <option value="${filteredParty.id}" ${filteredParty.id == superior ? "selected" : ""}>${filteredParty.company_name}</option>`
                                    })

                                } else {
                                    filteredParties[index].map((filteredParty, index) => {
                                        if (filteredParty.id == superior || filteredParty.superior == superior) {
                                            optionEntry += ` <option value="${filteredParty.id}" ${filteredParty.id == superior ? "selected" : ""}>${filteredParty.company_name}</option>`
                                        }
                                    })
                                }
                                if ("{{$getClientSetting->order_to}}" == 1) optionEntry += `</optgroup>`;
                                $('#orderTo').append(optionEntry).trigger('change');
                            })
                        }
                        if (data.length) {
                            const filteredParties = data;//parties.filter(party => data.includes(party.client_type));
                            filteredParties.map(filteredParty => {
                                $('#orderTo').append(` <option value="${filteredParty.id}" ${filteredParty.id == superior ? "selected" : ""}>${filteredParty.company_name}</option>`);
                            })
                        }

                        if(("{{$getClientSetting->order_to}}" == 0 && Object.entries(data).length == 0) || ("{{$getClientSetting->order_to}}" == 0 && !"{{$order->order_to}}")){
                            $('#orderTo').empty()
                            $('#orderTo').append(` <option "selected">${companyName}</option>`)
                            $('#orderTo').attr('disabled', 'disabled')
            ...
            if ("{{$order->order_to}}") {
                setOrderTo($("select[id='order_clients'] option:selected")[0].dataset.clienttype, "{{$order->order_to}}")
            } else {
                $('#orderTo').empty();
                setOrderTo($("select[id='order_clients'] option:selected")[0].dataset.clienttype, null)
            }
            // if("{{$order->order_to}}" && !$('#orderTo').val()){
            //   @if($orderTo) $('#orderTo').append(` <option value="{{$orderTo->id}}" selected>{{$orderTo->company_name}}</option>`); @endif
            // }
            $('select[id="order_clients"]').on('change', function (event) {
                removeSchemeOnChange();
                update_amounts();
                let selectedClientType = event.target.selectedOptions[0].dataset.clienttype;
                let superior = event.target.selectedOptions[0].dataset.superior;
                let order_to_setup = "{{$getClientSetting->order_to}}";
                $('#orderTo').empty();
                $('#orderTo').select2().trigger('change');
                $('#orderTo').append(' <option selected disabled>Select Order To</option>')

                const parties = @json($allClients);
                //Only superior
                if (order_to_setup == 0) {
                    const filteredParties = parties.filter(party => party.id == superior);
                    $('#orderTo').removeAttr('disabled')
                    $('#orderTo').trigger('change')

                    if (filteredParties.length) {
                        filteredParties.map(filteredParty => {
                            $('#orderTo').append(` <option value="${filteredParty.id}" selected>${filteredParty.company_name}</option>`);
                        })
                    } else {
                        $('#orderTo').empty();
                        $('#orderTo').append(` <option "selected">${companyName}</option>`)
                ...
                } else if (order_to_setup == 1) {
                    if (superior) {
                        $('#orderTo').removeAttr('disabled')
                        $('#orderTo').trigger('change')
                        // Superior and superior partyTypes
                        setOrderTo(selectedClientType, superior)
                    } else {
                        $('#orderTo').empty();
                        $('#orderTo').append(` <option "selected">${companyName}</option>`)
                        $('#orderTo').attr('disabled', 'disabled')
            ...
            if(enablePartyRate  || enablePartyCategoryRate){
            ...
            function validateCreditLimit(formEl) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.success) {
                            CKEDITOR.instances['order_note'].setReadOnly(true);
                    ...
                    error: function (error) {
                        if (error.responseJSON.errors.id) {
                            let err = error.responseJSON.errors.id[0];
                            alert(err);
                        }
                        $('#submitOrder').attr("disabled", false);
                    },
            ...
            $(".table").on("change", ".prodrow .product", function () {
                ...
                let categoryRateIdToApply = 0;
                let currentElement = $(this);
                let prod = parseInt(currentElement.val());

                if(enablePartyCategoryRate){
                  let categoryRates = $('#order_clients option:selected').data('categoryrates');
                  let prodCat = currentElement.select2().find(":selected").data("category");
                  if(prodCat && categoryRates.length > 0){
                      let appliedCategoryRates = categoryRates.map((el, ind) => {
                        if(el.category_id == prodCat) categoryRateIdToApply = el.id 
                      })
                  }
                }
                ...
                if (prod == prodValue) {
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            'enablePartyCategoryRate': enablePartyCategoryRate,
                            'categoryRateIdToApply' : categoryRateIdToApply,
                            'partyId': selClientId,
                            'rateId': rateId
                        },
                        beforeSend: function () {
                    ...
                    if ((enablePartyRate && rateId) || enablePartyCategoryRate) {
                        $.ajax({
                            type: 'POST',
                            async: true,
                            ...
                            success: function (data) {
                                ...
                                @else $(`#utype${rowid}`).text(unittype);
                                @endif
            ...
            $(".table").on("change", ".prodrow .productvariant", function () {
                ...
                if(enablePartyCategoryRate){
                  let categoryRates = $('#order_clients option:selected').data('categoryrates');
                  let prodCat = $(`#row${rowid}`).children().find('.product').select2().find(":selected").data("category");
                ...
                if ((enablePartyRate && rateId) || enablePartyCategoryRate) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            $(`#hidden_mrp${rowid}`).val(mrp_amount);
                            $(`#u_id${rowid}`).val(unit_id);
                            updateUnitSelection(unit_id, rowid,  $(`#row${rowid}`).children().find('.product').select2().find(":selected").val());
                            @endif
                            $(`#qty${rowid}`).val(qty);
                            $(`#amt${rowid}`).val((qty * mrp_amount).toFixed(2));
                            $(`#short_desc${rowid}`).val(short_desc);
            ...
            $(document).on('click', '.removeRowBtn', function () {
                ...
                if(!$(this).parent().parent().hasClass('oldprodrow')){
                  allowRemoveRow = true;
                }
                if (parseInt(orderRowCount) > 1 || allowRemoveRow) {
            ...
            $('body').on('click', '#submitOrder', function (e) {
                ...
                if (getRowCount > 1) {
                    ...
                    if (lastRow.find('.product').val() === "") {
                        lastRow.remove();
                        lastRow =  $("#dynamic_field").find("tr").last().data('rownum')
                        console.log(lastRow)
                        $(`#add${lastRow}`).removeClass("hidden")
                        return true
                    }
            ...
            let scheme_discount = {{ $scheme_off_value }};
            if (parseInt(scheme_discount) > 0) {
                $('#scheme-amount-off').show();
                $('#scheme-amount').val(parseFloat(scheme_discount));
            }
            update_amounts();
            let count_product_schemes = {{ count($free_schemes)}};
            if (count_product_schemes > 0) {
                $('#scheme-table-test').show()
                let product_schemes = @json($free_schemes);

                product_schemes.forEach((element) => {
                    // $('#scheme-table-test').show()
                    let id = element.id;
                    let product_name = element.product_name;
                    let product_variant = element.product_variant;
                    let free_unit = element.freeItem;
                    let unit = element.unit;
                    let row = `<tr>
                                    <td><select id="offered_product${id}"><option selected disabled>${product_name}</option></select></td>
                                    <td><select id="offered_variable${id}"><option selected disabled>${product_variant}</option></select></td>

                                     <td><select id="offered_variable_attribute${id}"><option selected disabled> </option></select></td>
                                       @if(getClientSetting()->order_with_amt == 0)
                    <td><div class="input-group contentFit"><span class="input-group-addon rs-symbol">
{{$getClientSetting['currency_symbol']}}
                    </span><input type="text" value="0.00" disabled class="input-group-addon mrp_addon form-control name_list mrp" ></div></td>
@endif
                    <td><div class="input-group contentFit"><input type="text" value="${free_unit}" disabled class="form-control name_list qty onlynumber"> <span class="input-group-addon per-symbol" id="utype1">${unit}</span></div></td>
                                        @if(getClientSetting()->order_with_amt == 0)
                    <td><div class="input-group contentFit"><span class="input-group-addon rs-symbol">{{$getClientSetting['currency_symbol']}}</span><input type="text" value="0.00" disabled class="input-group-addon mrp_addon form-control name_list mrp" ></div></td>
                                        <td><div class="input-group contentFit"><span class="input-group-addon rs-symbol">{{$getClientSetting['currency_symbol']}}</span><input type="text" value="0.00" disabled class="input-group-addon mrp_addon form-control name_list mrp"></div></td>
                                         @endif
                    </tr>`;
                    $('#scheme-table-test').append(row)
                    $(`#offered_product` + id).select2();
                    $(`#offered_product` + id).prop('disabled', true);
                    $(`#offered_variable` + id).select2({
                        placeholder: 'Select Variant'
                    });
                    $(`#offered_variable` + id).prop('disabled', true);
                    $(`#offered_variable_attribute` + id).select2({
                        placeholder: 'Select Attribute '
                    });
                    $(`#offered_variable_attribute` + id).prop('disabled', true);
            ...
            const freeItem = [];
            const discount = [];
            let previous_schemes = @json($old_values);
            previous_schemes.forEach((element) => {
                scheme.push(element.scheme_id);
                discount.push(element.discounts);
                freeItem.push(element.free_items);
            })

            $('#scheme_values').val(scheme)
            $('#scheme_discounts').val(discount)
            $('#scheme_free_items').val(freeItem)




            $(document).on('click', '.check-schemes', function () {
                let getCountRow = $(".prodrow").length;
                let num_of_field = $('#dynamic_field > tbody > tr').last().attr('data-rownum');
                if (num_of_field === 0) {
                    num_of_field = 1;
                }

                let client = $('#order_clients').val();
                let product_names = [];
                let variants_names = [];
                let quantity = [];
                let applied_rate = [];
                ...
                for (i = 0; i <= num_of_field; i++) {
                    ...
                    variant_name = $('#product_variant_name' + i).val()
                    rate = $('#rate' + i).val();
                    qty = $('#qty' + i).val()
                    applied_rate.push(rate)
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ domain_route('company.admin.get.all.available.schemes') }}",
                    data: {
                        'names': product_names,
                        'variant_names': variants_names,
                        'quantity': quantity,
                        'applied_rate': applied_rate,
                        'applied_schemes': $('#scheme_values').val(),
                        'client_id' : client,
                    },

                    success: function (data) {
                        if (data.count == 0) {
                            $("#apply-scheme").attr("disabled", true);
                        } else {
                            $("#apply-scheme").attr("disabled", false);
                        }
                        document.getElementById("schemes").innerHTML = data.html;
                    },
            ...
            $("#apply-scheme").on('click', function () {

                $('#scheme_values').val("")
                $('#scheme_discounts').val("")
                $('#scheme_free_items').val("")

                const scheme = [];
                const freeItem = [];
                const discount = [];
                $("input:checkbox[name=schemes]:checked").each(function () {

                    scheme.push($(this).val());
                    freeItem.push($(this).data('freeitem'));
                    discount.push($(this).data('discount'));
                });
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ domain_route('company.admin.apply.selected.scheme') }}",
                    data: {
                        scheme: scheme,
                        freeItem: freeItem,
                        discount: discount
                    ...
                    success: function (data) {
                        ...
                        data.schemes.forEach((element, index, array) => {
                            scheme.push(element.id);
                            freeItems.push(element.freeItem);
...
{{$getClientSetting['currency_symbol']}}
                                ...
                                @endif
                                <td><div class="input-group contentFit"><input type="text" value="${free_unit}" disabled class="input-group-addon mrp_addon form-control name_list mrp"> <span class="input-group-addon per-symbol" id="utype1">${unit}</span></div></td>
                            @if(getClientSetting()->order_with_amt == 0)
                                <td><div class="input-group contentFit"><span class="input-group-addon rs-symbol">{{$getClientSetting['currency_symbol']}}</span><input type="text" value="0.00" disabled class="input-group-addon mrp_addon form-control name_list mrp" ></div></td>
                                    <td><div class="input-group contentFit"><span class="input-group-addon rs-symbol">{{$getClientSetting['currency_symbol']}}</span><input type="text" value="0.00" disabled class="input-group-addon mrp_addon form-control name_list mrp"></div></td>
                                    @endif
                                ...
                                discount_amount = discount_amount + parseFloat(element.discount);
                            }
                        ...
                        $('#scheme-amount').val(parseFloat(discount_amount).toFixed(2));
                        $('.subtotal').val(parseFloat(sub_total) - discount_amount)
          ...
          <div class="col-xs-12">
            <input type="text" class="form-control tax" id="tax${optionId}" placeholder="${optionPercent}"
              value="" readonly>
                ...
                @endif
                let scheme_discount = parseFloat($('#scheme-amount').val());


                if (isNaN(scheme_discount)) {
                    scheme_discount = 0
                }
                if (subtotal < scheme_discount) {
                    alert("Total amount cannot be negative, this scheme cannot be applied");
                    let initial_value = parseFloat($('#scheme-amount').val());
                    let discount_value = scheme_discount;
                    let final = initial_value - discount_value
                    $('#scheme-amount').val(final)

                    if ($('#scheme-amount').val() === '0') {
                        $('#scheme-amount-off').hide()
                    }
                } else {
                ...
                @elseif ($order->product_level_discount_flag == 1)
                    total = subtotal - discountTotAmount;
                @endif
        ...
        function removeSchemeOnChange() {
            $("#scheme-table-test > tbody").html("");
            $('#scheme-table-test').hide()
            $('#scheme-amount-off').hide()
            $('#scheme-amount').val(0);
            if ($('#scheme_values').val()) {
                $('#scheme-alert').show();
            }
            $('#scheme_values').val("")
            $('#scheme_discounts').val("")
