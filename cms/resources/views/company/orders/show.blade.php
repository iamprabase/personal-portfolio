                    ...
                    <div class="box box-default">
                        ...
                        <div id="printArea">
                            <div class="box-header with-border">
                                <a href="{{ URL::previous() }}" class="btn btn-default btn-sm" id="backBtn"> <i
                            ...
                            <div class="box-header with-border">
                                ...
                                <div class="page-action pull-right">
                                    {{-- {!!$action!!} --}}
                                    <a href="{{ domain_route('company.admin.order.download.newInvoice',[$order->id])}}"
                                       class="btn btn-default btn-sm"
                                       id="downloadBtn"> <i class="fa fa-download"></i>Download</a>


{{--                                    <a href="{{ domain_route('company.admin.order.download',[$order->id])}}"--}}
{{--                                       class="btn btn-default btn-sm"--}}
{{--                                       id="downloadBtn"> <i class="fa fa-download"></i> Download</a>--}}
                                    <a class="btn btn-default btn-sm" data-toggle="modal" data-target="#myModal"
                                       id="mailBtn"> <i
                                                class="fa fa-envelope"></i> Mail</a>
                                    <button class="btn btn-default btn-sm" onclick="printSingleOrder();" id="printBtn">
                                ...
                                <div id="detail_div">
                                    <div class="row">
                                        ...
                                        <div class="col-xs-{{$col_gap}}">
                                            <div class="order-dtl-bg"> To: {{ getClient($order->client_id)['name'] }}
                                                <br>
                                                Address:
                                            ...
                                            <div class="col-xs-{{$col_gap}}">
                                                <div class="order-dtl-bg">
                                                    ...
                                                    @if(config('settings.ageing')==1 && Auth::user()->can('ageing-view'))
                                                        Due
                                                        Date: {{$order->due_date?getDeltaDate(date('Y-m-d', strtotime($order->due_date))):null}}
                                                    @endif
                                                </div>
                                            </div>
                                        @else
                                            ...
                                            <div class="col-xs-{{$col_gap}}">
                                                <div class="order-dtl-bg">
                                                    ...
                                                    @if($order->order_to)
                                                        ...
                                                        <br/>
                                                        Address:
                                                        {{ getClient($order->order_to)['address_1'] }}
                                                        <br/>
                                                        ...
                                                        <br/>
                                                        Mobile: {{ getClient($order->order_to)['mobile'] }}
                                                    @else
                                    ...
                                    <table border="1px" id="example" class="display table" width="100%" cellspacing="0">
                                        ...
                                        @if($getClientSetting->order_with_amt==0)
                                            @if($scheme_off_value > 0)
                                                <tr>
                                                    <td colspan="{{$colspan}}" class="text-right"><b>Scheme amount
                                                            off</b></td>
                                                    <td>
                                                        <b>{{ config('settings.currency_symbol')}} {{ number_format($scheme_off_value,2)}}</b>
                                                    </td>
                                                </tr>
                                            @endif
                                            <tr>
                                                ...
                                                <td>
                                                    <b>{{ config('settings.currency_symbol')}} {{ number_format($order->tot_amount,2)}}</b>
                                                </td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <b>{{ config('settings.currency_symbol')}} {{ number_format($order->total,2)}}</b>
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <b>{{ config('settings.currency_symbol')}} {{ number_format($order->grand_total,2)}}</b>
                                                </td>
                                            </tr>
                                        @endif
                                        </tbody>
                                    ...
                                    @if(count($free_schemes))
                                        <div class="row">
                                            <div class="col-md-6">
                                                <table border="1px" id="example" class="display table" width=50%"
                                                       cellspacing="0">
                                                    <thead>
                                                    <tr>

                                                        <th>Free Scheme Products</th>
                                                        <th>Qty</th>
                                                    </tr>
                                                    </thead>
                                                    ...
                                                    @foreach($free_schemes as $schemes)
                                                        <tr>
                                                            <td>
                                                                {{ $schemes['product_name']}} @if(!empty($schemes['product_variant'])) {{ $schemes['product_variant'] }} @endif
                                                            </td>

                                                            <td>{{$schemes['freeItem']}} {{$schemes['unit_name']}}</td>
                                    ...
                                    @if(count($schemeNames))
                                        <strong>Applied Schemes :</strong>
                                        @foreach($schemeNames as $key => $name)
                                            <p>{{$key+1}}. {{ $name }}</p>
                                        @endforeach
                                    @endif
                                        ...
                                        <div class="row">
                                            <div class="col-xs-6">
                                                <div class="order-dtl-bg">
                                                    ...
                                                    <br>
                                                    Dispatch
                                                    Place: {{ !empty($order->delivery_place)?$order->delivery_place:'N/A' }}
                                                    <br>
                                                    Dispatch
                                                    Note: {{ !empty($order->delivery_note)?strip_tags($order->delivery_note):'N/A' }}
                                                    <br>
                                                    Transport
                                                    No.: {{ !empty($order->transport_number)?strip_tags($order->transport_number):'N/A' }}
                                                    <br>
                                                    Transport
                                                    Name: {{ !empty($order->transport_name)?strip_tags($order->transport_name):'N/A' }}
                                                    <br>
                                                    Bilty
                                                    Number: {{ !empty($order->billty_number)?strip_tags($order->billty_number):'N/A' }}
                                                </div>
                                            </div>
                                        </div>
                                    @endif
                        ...
                        <div class="box-body order-location">
                            ...
                            <p>
                                ...
                                {!!isset($convertedDate)?"- <b>".$convertedDate."</b>":"<b>".date('h:i A', strtotime($order->order_datetime))."</b>"!!}
                                    ...
                                    @else {{ getEmployee($order->employee_id)['name'] }} @endif</b> .<span>@if(isset($order->latitude) && isset($order->longitude))
                                        <i class="fa fa-map-marker" aria-hidden="true" onclick="showModal();"></i>@endif</span>
                            </p>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div id="errors">
                        @if ($errors->has('email')) <p
                                class="help-block has-error">{{ $errors->first('email') }}</p> @endif
                    </div>
                </div>
