...
<style>
  ...
  .box-header__head + .btn {
    margin-left: auto;
    margin-right:8px;
  }

  /* .box-header__head ~ .btn:last-child{
    margin-left:8px;
  }
   */
    ...
    <div class="row">
      <div class="col-12 col-sm-6 col-md-3" id="ticker1">
        <span class="pull-right">
          <select id="opt1" style="width:20px;background-color:white;border:white;cursor:pointer;">
            <option value="0" oth="ses"></option>
          </select>
        </span>
        <div class="info-box">
          <span class="info-box-icon bg-aqua elevation-1"><i class="fa fa-shopping-cart"></i></span>
          <div class="info-box-content">
            <span class="info-box-text" id="col1name"></span>
            <span class="info-box-number" id="col1val"></span>
          </div>
        </div>
      </div>
      <div class="col-12 col-sm-6 col-md-3" id="ticker2">
        ...
        <div class="info-box mb-3">
          ...
          <div class="info-box-content">
            <span class="info-box-text" id="col2name"></span>
            <span class="info-box-number" id="col2val"></span>
          </div>
        </div>
      </div>
      <div class="col-12 col-sm-6 col-md-3" id="ticker3">
        ...
        <div class="info-box mb-3">
          ...
          <div class="info-box-content">
            <span class="info-box-text" id="col4name"></span>
            <span class="info-box-number" id="col4val"></span>
          </div>
        </div>
      </div>
        ...
        <div class="box box-default" id="qtygraph1">
          <div class="box-header with-border">
            <div class="box-header__head">
              <h3 class="box-title" id="qtytitle_opt1">Top Products By No. of Categories Sold</h3>
              <div class="box-tools">
                <div class="btn-group">
            ...
            @if(config('settings.order_with_amt')==0)
              <a class="btn btn-primary" id="ordval_n" custattr="products" onclick="qtytypefind('ordval')">By value</a>
            @endif
            <a class="btn btn-primary" id="unitssold_n" custattr="products" onclick="qtytypefind('unitssold')">By units sold</a>
          </div>
          <div class="box-body">
...
@if((config('settings.orders')==1 && config('settings.order_with_amt')==0)  || config('settings.collections')==1)
...
@if((config('settings.party')==1 && config('settings.collections')==1) || (config('settings.party')==1 && config('settings.orders')==1 && config('settings.order_with_amt')==0))
          ...
          <div class="box-header with-border">
            ...
            @if(config('settings.collections')==1)
              @if(Auth::user()->can('collection-view'))
                <a class="btn btn-primary pull-right" id="parties_collvalamt" onclick="parties_collamt()">By Collection</a>
              @endif
            @endif
            @if(config('settings.orders')==1)
              @if(Auth::user()->can('order-view'))
                @if(config('settings.order_with_amt')==0)
                  <a class="btn btn-primary pull-right" id="parties_ordvalamt" onclick="parties_ordamt()">By Order Value</a>
                @endif
              @endif
            @endif
          </div>
...
@if((config('settings.beat')==1 && config('settings.orders')==1 && config('settings.order_with_amt')==0) || (config('settings.beat')==1 && config('settings.collections')==1))
      ...
      <div class="col-md-12">
        <div class="box box-default" id="beats_divcollordamt">
          <div class="box-header with-border">
            ...
            @if(config('settings.collections')==1 )
              @if(Auth::user()->can('collection-view'))
                <a class="btn btn-primary pull-right" id="beats_collvalamt" onclick="topbeats(this)">By Collection</a>
              @endif
            @endif
            @if(config('settings.orders')==1 )
              @if(Auth::user()->can('order-view'))
                @if(config('settings.order_with_amt')==0)
                  <a class="btn btn-primary pull-right"  id="beats_ordvalamt" style="margin-right:8px;" onclick="topbeats(this)">By Order Value</a>
                @endif
                <a class="btn btn-primary pull-right"  id="beats_ordval" onclick="topbeats(this)">By No. of Orders</a>
              @endif
            @endif
          </div>
          <div class="box-body">
...
<div class="row" style="min-height:380px">
  ...
  @if((config('settings.retailer_app')==1) && (config('settings.orders')==1) && (config('settings.party')==1))
    @if(Auth::user()->can('outlet-view') && Auth::user()->can('order-view') && Auth::user()->can('party-view'))
      <div class="col-md-6">
...
<script>   
    ...
    $(document).on('click',".applyBtn",function () {
      ...
      var customval = 'custom##'+startDate+'##'+endDate;
      savedatefilter('storeddate',customval);
      countParameters(startDate, endDate, type="");
      getOtherGraphs(startDate, endDate, type="");
      addClientVisitMarker(startDate, endDate, type="");
    });
    ...
    $(document).ready(function(){
      @if(config('settings.ncal')==1)
        ...
        addClientVisitMarker(startDate, endDate, type="nepali");
      @endif
    ...
    function countParameters(startDate, endDate, type){
      $.post("{{ domain_route('company.admin.fetchtickercountdata') }}",{start_date: startDate, end_date: endDate, type},function(res){
        if(res!='' || res!='undefined' && (typeof(res.data)=='object')){
          ...
          $.post("{{ domain_route('company.admin.analyticssave') }}", {searchstr: 'ticker_'}, function(response){
            if(response.data.chktype=='notavailable'){
                @if(Auth::user()->can('order-view'))
                  $("#ticker1").find(".changeTicker1_totord").click();
                @endif
                @if(Auth::user()->can('zeroorder-view'))
                  $("#ticker4").find(".changeTicker4_nozoord").click();
                @endif
            }else{
                var ticker1 = response.data.ticker_1;
                var ticker2 = response.data.ticker_2;
                var ticker3 = response.data.ticker_3;
                var ticker4 = response.data.ticker_4;
                if(ticker1!==undefined){
                  $("#ticker1").find(".changeTicker1_"+ticker1).click();
                }
                if(ticker2!==undefined){
                  $("#ticker2").find(".changeTicker2_"+ticker2).click();
                }
                if(ticker3!==undefined){
                  $("#ticker3").find(".changeTicker3_"+ticker3).click();
                }
                if(ticker4!==undefined){
                  $("#ticker4").find(".changeTicker4_"+ticker4).click();
                }
            }
          });
    ...
    function addClientVisitMarker(startDate, endDate, type=''){
    ...
    function savedatefilter(datatyp,dater){
      $.post("{{ domain_route('company.admin.analyticssavedtfilter') }}", {searchstr: datatyp, datefiltertype: dater}, function(response){
      });
    }
    ...
    jQuery(function($) {
      ...
      @if(config('settings.ncal')==0)
        ...
        @if($storeddate['chktype']=='available')
          var dater = "{{ $storeddate['mapvalue'] }}";
          if(dater=="today"){
            startDate = moment();
            endDate = moment();
          ...
          }else if(dater=="last7day"){ 
            startDate = moment(moment().subtract(6, 'days'));
            endDate = moment();
          }else if(dater=="last30day"){ 
            startDate = moment(moment().subtract(29, 'days'));
            endDate = moment();
          }else if(dater=="thismonth"){  
            startDate = moment(moment().startOf('month'));
            endDate = moment();
            actdates = startDate+'#'+endDate;
          }else if(dater=="lastmonth"){ 
            startDate = moment(moment().subtract(1, 'month').startOf('month'));
            endDate = moment().subtract(1, 'month').endOf('month');
          }else{
            var separatedate = dater.split('##');
            startDate = moment(separatedate[1],'YYYY-MM-DD');
            endDate = moment(separatedate[2],'YYYY-MM-DD');
          }
        @else
          startDate = moment().subtract(30, 'days');
          endDate = moment();
        @endif
        window.sessionStorage.setItem('startdate',startDate.format("YYYY-MM-DD"));
        window.sessionStorage.setItem('enddate',endDate.format("YYYY-MM-DD"));
        countParameters(startDate.format("YYYY-MM-DD"), endDate.format("YYYY-MM-DD"), type="");
        getOtherGraphs(startDate.format("YYYY-MM-DD"), endDate.format("YYYY-MM-DD"), type="");
        addClientVisitMarker(startDate.format("YYYY-MM-DD"), endDate.format("YYYY-MM-DD"), type="");

        var start_t = startDate;//moment().subtract(30, 'days');
        var end_t = endDate;//moment();
        ...
        $(".ranges ul li").on('click', function() {
          ...
          switch (dataRange) {
            ...
            case "Today":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','today');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
            case "Yesterday":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','yesterday');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
            case "Last 7 Days":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','last7day');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
            case "Last 30 Days":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','last30day');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
            case "This Month":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','thismonth');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
            case "Last Month":
              ...
              window.sessionStorage.setItem('startdate',startDate);
              window.sessionStorage.setItem('enddate',endDate);
              savedatefilter('storeddate','lastmonth');
              getOtherGraphs(startDate, endDate, type="");
              countParameters(startDate, endDate, type="");
              addClientVisitMarker(startDate, endDate, type="");
              break;
    ...
    function getOtherGraphs(startDate, endDate, type=""){
      ...
      @if(config('settings.party')==1)
        @if(Auth::user()->can('party-view'))
          @if(config('settings.orders')==1 || config('settings.collections')==1)
            @if(Auth::user()->can('order-view') && config('settings.order_with_amt')==0)
              $("#parties_ordvalamt").trigger('click');
            @elseif(Auth::user()->can('collection-view'))
      ...
      linegraph1(startDate, endDate, type).then(function(){
        $('#linegrdim1').removeClass('box-loader');
        $('#linegraph1_loader').attr('hidden', 'hidden');
      });

      linegraph2(startDate, endDate, type).then(function(){
    ...
    function qtyProdBraCatGraph(divid, dates, values,gtype) {
      var xtext = ytext = ''; 
      if(gtype=='prod'){
        xtext = 'Products'; ytext = 'Units';
      }else if(gtype=='cat'){
        xtext = 'Categories'; ytext = 'Units';
      }else if(gtype=='bra'){
        xtext = 'Brands'; ytext = 'Units';
      }else if(gtype=='beats'){
    ...
    function plotLineGraphs2(dates, totalorder, totcoll, amtcurrency){
      ...
      Highcharts.chart('linegraphs2', {
          ...
          yAxis: {
              ...
              labels: {
                formatter: function () {
                    return amtcurrency + this.axis.defaultLabelFormatter.call(this);
                }            
              }
          },
          xAxis: {
    ...
    function timeSpentVistGraph(dates, values) {
      ...
      Highcharts.chart('time_visit', {
          ...
          yAxis: {
              ...
              labels: {
                formatter: function () {
                  var yaxislabel = this.axis.defaultLabelFormatter.call(this);
                  var searchres = yaxislabel.search('k');
                  if(searchres>-1){
                    searchres = (parseFloat(yaxislabel))*1000;
                    hourdat = (searchres/3600).toFixed(2);
                  }else{
                    if(yaxislabel!='0'){
                      hourdat = (yaxislabel/3600).toFixed(2);
                    }else{
                      hourdat = yaxislabel;
                    }
                  }
    ...
    function topPartiesGraph(graphtype, dates, values, currsymbol) {
      ...
      Highcharts.chart('parties_collordamt', {
        ...
        yAxis: {
          ...
          labels: {
              formatter: function () {
                  return  currsymbol + this.axis.defaultLabelFormatter.call(this);
              }            
          },
          allowDecimals: false,
        },
    ...
    function topOutletsGraph(dates, values) {
      ...
      if(finaldata.length==2){
        testt[0] = finaldata[1];
        // testt[0].y = testt[0].y+((85.5/100)*testt[0].y);
        // tp = finaldata[1].y+finaldata[0].y;
        testt[1] = finaldata[0];
        // testt[1].y = tp;
      }
      var chk = Math.round((dat[1]/(dat[0]+dat[1]))*100,2);
    ...
    function topBeatsAmtGraph(dates, values, currsymbol, graphtype) {
      ...
      Highcharts.chart('beats_collordamt', {
        ...
        yAxis: {
          ...
          labels: {
              formatter: function () {
                  return cursymb + this.axis.defaultLabelFormatter.call(this);
              }            
          },
          allowDecimals: false,
        },
    ...
    function ordersg1(startDate,endDate,type){
      var retdata = [];
      totOrderDates = totOrderValues =  [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => { 
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchtotal_order') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            ...
            retdata['totOrderDates'] =  totOrderDates;
            retdata['totOrderValues'] =  totOrderValues;
            resolve(retdata)
          }
        });
      });
    ...
    function zeroordersg2(startDate,endDate,type){
      var retdata = [];
      noOrderDates = noOrderValues =  [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchzero_order') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            if(response.data!='nok'){
              if (type == undefined || type == "") {
            ...
            retdata['noOrderDates'] =  noOrderDates;
            retdata['noOrderValues'] =  noOrderValues;
            resolve(retdata);
          }
        });
      })
    ...
    function totalcallsg3(startDate,endDate,type){
      var retdata = [];
      totCallsDates = totCallsValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          ...
          success:function(response){
            ...
            retdata['totCallsDates'] =  totCallsDates;
            retdata['totCallsValues'] =  totCallsValues;
            resolve(retdata)
          }
        });
      })
    ...
    function visitsg4(startDate,endDate,type){
      var retdata = [];
      totVisitsDates = totVisitsValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchtotalvisits') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            if(response.data!='nok'){
              if (type == undefined || type == "") {
            ...
            retdata['totVisitsDates'] =  totVisitsDates;
            retdata['totVisitsValues'] =  totVisitsValues;
            resolve(retdata)
          }
        });
      })
    ...
    function productssoldg5(startDate,endDate,type){
      var retdata = [];
      totProdSoldDates = totProdSoldValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchtotprodsold') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            if(response.data!='nok'){
              if (type == undefined || type == "") {
            ...
            retdata['totProdSoldDates'] =  totProdSoldDates;
            retdata['totProdSoldValues'] =  totProdSoldValues;
            resolve(retdata)
          }
        });
      })
    ...
    function newpartiesaddedg6(startDate,endDate,type){
      var retdata = [];
      noPartiesAddedDates = noPartiesAddedValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchtotpartiesadded') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            if(response.data!='nok'){
              if (type == undefined || type == "") {
            ...
            retdata['noPartiesAddedDates'] =  noPartiesAddedDates;
            retdata['noPartiesAddedValues'] =  noPartiesAddedValues;
            resolve(retdata)
          }
        });
      })
    ...
    async function linegraph1(startDate, endDate, type) {
      ...
      g1 = await ordersg1(startDate,endDate,type);
      g2 = await zeroordersg2(startDate,endDate,type);
      g3 = await totalcallsg3(startDate,endDate,type);
      g4 = await visitsg4(startDate,endDate,type);
      g5 = await productssoldg5(startDate,endDate,type);
      g6 = await newpartiesaddedg6(startDate,endDate,type);
      totOrderDates = g1['totOrderDates'];
      totOrderValues = g1['totOrderValues'];
      noOrderDates = g2['noOrderDates'];
      noOrderValues = g2['noOrderValues'];
      totCallsDates = g3['totCallsDates'] ;
      totCallsValues = g3['totCallsValues'];
      totVisitsDates = g4['totVisitsDates'];
      totVisitsValues = g4['totVisitsValues'];
      totProdSoldDates = g5['totProdSoldDates'];
      totProdSoldValues = g5['totProdSoldValues'];
      noPartiesAddedDates = g6['noPartiesAddedDates'];
      noPartiesAddedValues = g6['noPartiesAddedValues'];
    ...
    function ordervalueamtg1(startDate,endDate,type){
      var retdata = [];
      totOrderAmtDates = totOrderAmtValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          url: "{{ domain_route('company.admin.fetchtotalorderamt') }}",
          data: {start_date: startDate, end_date: endDate, type},
          success:function(response){
            if (type == undefined || type == "") {
            ...
            totOrderAmtValues = response.data.values;
            retdata['totOrderAmtDates'] =  totOrderAmtDates;
            retdata['totOrderAmtValues'] =  totOrderAmtValues;
            resolve(retdata)
          }
        });
      })
    ...
    function callectionvalueamtg2(startDate,endDate,type){
      var retdata = [];
      totCollAmtDates = totCollAmtValues = [];
      let diffInDays = daysDiff(new Date(startDate), new Date(endDate));
      return new Promise((resolve,reject) => {
        $.ajax({
          method: 'post',
          ...
          success:function(response){
            ...
            retdata['totCollAmtDates'] =  totCollAmtDates;
            retdata['totCollAmtValues'] =  totCollAmtValues;
            retdata['amtCurrency'] =  amtCurrency;
            resolve(retdata)
          }
        });
      })
    ...
    async function linegraph2(startDate, endDate, type){
      ...
      let amtCurrency = ''; 
      let h1 = h2 = [];
      h1 = await ordervalueamtg1(startDate,endDate,type);
      h2 = await callectionvalueamtg2(startDate,endDate,type);
      totOrderAmtDates = h1['totOrderAmtDates'];
      totOrderAmtValues = h1['totOrderAmtValues'];
      totCollAmtDates = h2['totCollAmtDates'];
      totCollAmtValues = h2['totCollAmtValues'];
      amtCurrency = h2['amtCurrency'];
      data_check1 = 0;
