<div class="col-xs-12 site-tital" style="padding-left: 0px;">
  ...
  <div class="col-xs-4 pull-right" style="margin-top:25px;">
      <span>
        <strong class="filesmaxLimit {{$file_count<20 ? 'hidden': ''}}" style="color: #fbb002fa;">Maximum limit of 20 items reached.</strong>
        <input type="hidden" class="filesCount" value="{{$file_count}}">
        @if(Auth::user()->can(\Illuminate\Support\Str::singular($type).'uploads-create'))  
        <a href="#" class="filesLink {{$file_count>=20 ? 'hidden': ''}}" data-toggle="modal" data-target="#{{$type}}FileCreateModal">
...
<div class="info col-xs-12" style="height: 700px;overflow-y: scroll;">
  <div class="box-body">
      <div id= "filePartial" class="{{$type}}View">
        <table class="table table-bordered table-striped {{$type}}PartialTable">
          <thead>
              <tr>
                  <th>S.No.</th>
                  <th>FileName</th>
          ...
          <div class="form-group @if ($errors->has('name')) has-error @endif">
              <div class="col-xs-3 imgUp">
                ...
                <label class="btn btn-default"> Select File 
                  <input id="newChosenUpload" type="file" name="chosenUpload[]" class="uploaderInput img" data-type="{{$type}}" value="Upload Photo" style="width: 0px;height: 0px;overflow: hidden;" required>
                </label>
...
<div class="modal modal-default fade fileDeleteModal" id="{{$type}}FileDeleteModal" tabindex="-1" unit="dialog"
