...
<section class="content">
  ...
  <div class="box box-default">
    ...
    {!! Form::open(array('url' => url(domain_route("company.admin.client.store", ["domain" => request("subdomain")])), 'method' => 'post', 'files'=> true, 'id' =>'partyform', 'autocomplete'=>"off")) !!}

    @include('company.clients._form')

      {!! Form::submit('Create', ['class' => 'btn btn-primary pull-right keySubmit', 'id' => 'create_new_entry']) !!}
...
<script>
  ...
  $(document).on("focusout", '#mobile', function(e)  {
      ...
      $('select[name="client_type"]').on('change', function () {
          ...
          if (client_type) {
              $.ajax({
                  url: '/admin/client/getsuperiorlist?client_type=' + client_type,
                  type: "GET",
                  dataType: "json",
                  cache: false,
                  success: function (data) {
                      ...
                      if(data.length==0) $("#superior").prop('disabled', true);
      ...
      $('select[name="client_type"]').change();
  ...
  //  alert(imggroupid);
   ...
   // alert(Imgcount);
    if(Imgcount < 3){
      ...
      $(this).closest(".form-group").find('.imgAdd').before('<div class="col-xs-4 imgUp"><div class="imagePreview"></div><label class="btn btn-primary">Upload<input name="'+inputname+'" type="file" class="uploadFile img" value="Upload Photo" style="width:0px;height:0px;overflow:hidden;"></label><i class="fa fa-times del"></i></div>');
    }else{
      $(".imgAdd").hide();
    }
  });
...
// var checkedCategoryId = new Array()
