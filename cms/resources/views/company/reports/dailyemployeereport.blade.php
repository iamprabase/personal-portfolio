    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <div class="col-xs-4">
            <h3 class="box-title">Daily Employee Report</h3>
          </div>
          <div class="col-xs-4">
            <strong>
          ...
          <div class="col-xs-4">
            <span id="dailyemployeereportexports" class="pull-right"></span>
          </div>
        </div>
        <!-- /.box-header -->
        <div class="box-body">
            ...
            <div class="col-xs-7">
              <div class="row">
                <div class="select-2-sec">
                  <div class="col-xs-4">
                    <div style="margin-top:10px;" id="employeefilter">
                      <select id="employee_filter" class="employee_filters select2 hidden">
                        <option></option>
                        <option value="null">All</option>
                        @forelse($employeesWithOrdersCollections as $id=>$name )
          ...
          <div id="mainBox">
            <table id="dailyemployeereport" class="table table-bordered table-striped">
              <thead>
                <tr>
                  <th>S.No.</th>
                  <th>Date</th>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    @if(config('settings.ncal')==0)
      ...
      $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
        ...
        $('#end_edate').val(end);
        var employeeVal = $('.employee_filters').find('option:selected').val();
        if(employeeVal=="null"){
          employeeVal = null;
        }
        ...
        if(startD != '' || endD != ''){
          $('#dailyemployeereport').DataTable().destroy();
  ...
  function initializeDT(employeeVal=null, startD, endD){
    const table = $('#dailyemployeereport').DataTable({
      language: {
        search: "_INPUT_",
      ...
      "ajax":{
        "url": "{{ domain_route('company.admin.dailyemployeereportDataTable') }}",
        "dataType": "json",
        "type": "POST",
        "data":{ 
          _token: "{{csrf_token()}}",
    ...
    table.buttons().container().appendTo('#dailyemployeereportexports');
      ...
      dt.one('preXhr', function (e, s, data) {
        ...
        dt.one('preDraw', function (e, settings) {
          if(button[0].className=="btn btn-default buttons-pdf buttons-html5"){
            var moduleName = "dailyemployeereport";
            var columnsArray = [];
            columnsArray.push("Date", "Employee Name", "Order Amount", "Collection Amount");
    ...
    function customExportAction(config, settings, modName, cols){
      ...
      $('#columns').val(cols);
      var propertiesArray = [];
      propertiesArray.push("id","name", "date", "order_total_amount", "collection_total_amount");
