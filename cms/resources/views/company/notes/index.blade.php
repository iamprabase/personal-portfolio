    ...
    <div class="col-xs-12">
      ...
      <div class="box">
        <div class="box-header">
          ...
          <a href="{{ domain_route('company.admin.notes.create') }}" class="btn btn-primary pull-right"
             style="margin-left: 5px;">
            <i class="fa fa-plus"></i> Create New
          </a>
          @endif
          <span id="noteexports" class="pull-right" style="margin-right: 20px !important;"></span>
        </div>
        <!-- /.box-header -->
                ...
                <div class="select-2-sec">
                  <div class="col-xs-3">
                    <div style="width:150px;margin-top:10px;height: 40px;z-index: 999 " id="partyfilter">
                      <select id="party_filter" class="party_filters select2 hidden">
                        ...
                        @forelse($clients as $id=>$party_name )
                  ...
                  <div class="col-xs-3">
                    <div style="width:150px;margin-top:10px;height: 40px;z-index: 999 " id="salesmfilter">
                       <select id="salesman_filter" class="employee_filters select2 hidden">
                        <option ></option>
                        <option value="null">All</option>
                        @forelse($employees as $id=>$employee_name )
...
<div id="myImgModal" class="modal custommodal">
  {{-- <div class="titleCaption"><span>Uploaded By: qwww</span><span>Last Updated: 31 Dec 2020</span> <br> </div> --}}
  <span class="close zoom-close">×</span>
  <div style="display: flex;margin: 0 200px;">
    <div href="#" class="previous round" style="cursor:pointer;font-size: 168px;margin: 180px 0;color: #fff;" key="">‹</div>;
    <img class="modal-content zoom-modal-content" id="img01" style="bottom: 30px;" src="">
    <div href="#" class="next round" style="cursor:pointer;font-size: 168px;margin: 180px 0;color: #fff;"  key="">›</div>;
  </div> 
  <div id="caption"></div>
</div>
...
<script>
    $(document).ready(function () {
        var activeRequestsTable = $('#note').DataTable();
  ...
  $(function () {
    ...
    @if(config('settings.ncal')==0)
      ...
      $('#reportrange').on('apply.daterangepicker', function(ev, picker) {
        ...
        sessionStorage.setItem('DT_Note_filters', JSON.stringify({
          "empVal": empVal,
          "partyVal": partyVal,
          "start": start,
          "end": end,
    ...
    var empVal = null;
    var partyVal = null;
    @if (\Session::has('DT_Note_filters'))
      let filtersSel = @json(\Session::get('DT_Note_filters'));
      sessionStorage.setItem('DT_Note_filters', filtersSel);
    @else
      sessionStorage.setItem('DT_Note_filters', "");
    @endif
    if(sessionStorage.getItem('DT_Note_filters')!="" || !sessionStorage.getItem('DT_Note_filters')==undefined ){
      let filterValue = JSON.parse(sessionStorage.getItem('DT_Note_filters'));
      if(filterValue){
  ...
  function showImage(el, index){
    ...
    var nextSibbling = el.nextElementSibling?.dataset.key;
    var previousSibbling = el.previousElementSibling?.dataset.key;
    if(!nextSibbling){
      $('.custommodal').find('.next').hide()
    }else{

      $('.custommodal').find('.next').show()
    }
    if(!previousSibbling){
      $('.custommodal').find('.previous').hide()
    }else{

      $('.custommodal').find('.previous').show()
    }
    if(!nextSibbling && !previousSibbling){
       $('.custommodal').find('.next').hide()
       $('.custommodal').find('.previous').hide()
       return;
    }
    $('.custommodal').find('.next').attr("key", nextSibbling)
    $('.custommodal').find('.previous').attr("key", previousSibbling)
    
  }
  function viewImage(el, index){
    $('.custommodal').modal('show');
    $('.custommodal').find('.next').show()
    $('.custommodal').find('.previous').show()
    $('.custommodal').find('.next').attr("key", "")
    $('.custommodal').find('.previous').attr("key", "")
    showImage(el, index)
  }
  
  $(document).find('.next').click(function(){
    let next = $(this).attr("key");
    if(next){
      
      showImage(document.getElementById(`img${next}`), next)
    }
  })
  
  $(document).find('.previous').click(function(){
    let previous = $(this).attr("key");
    if(previous){

      showImage(document.getElementById(`img${previous}`), previous)
    }
  })
  window.addEventListener('keyup', function(e){
    if($('.custommodal').is(':visible')) {
      var key = e.keyCode;
      if (key == 37) { 
        $(document).find('.previous').click();
      }else if(key == 39){
        $(document).find('.next').click();
      }
    }
  });
  ...
  function initializeDT(empVal=null, partyVal=null, startD, endD){
    const table = $('#note').DataTable({
      ...
      "ajax":{
        "url": "{{ domain_route('company.admin.notes.dataTable') }}",
        "dataType": "json",
        "type": "POST",
        "data":{ 
          _token: "{{csrf_token()}}", 
    ...
    table.buttons().container().appendTo('#noteexports');
