      ...
      <div class="col-xs-12">
        ...
        <div class="box">
          <div class="box-header">
            ...
            <a href="{{ route('app.plan.create') }}" class="btn btn-primary pull-right" style="margin-left: 5px;">
          ...
          <div class="box-body">
            <table id="plans" class="table table-bordered table-striped">
              ...
              @foreach($plans as $plan)
                ...
                <tr>
                  <td>{{ $i }}</td>
                  <td>{{ $plan->name}} {{($plan->custom!=0)?" (Custom)":""}}</td>
                  {{-- <td>{{ $plan->users}} {{ str_plural('User',$plan->users)}}
                    / {{ $plan->duration}} {{ str_plural($plan->duration_in,$plan->duration)}}</td>
                  <td> {{ $plan->companies()->with('plans')->get()->count() }} </td> --}}
                  <td>{{ strip_tags($plan->description) }}</td>
                  <td>
                    @if($plan->status =='Active')
                      <span class="label label-success">{{ $plan->status}}</span>
                    @else
                      <span class="label label-danger">{{ $plan->status}}</span>
                    @endif
                  </td>
                  <td>
                    <a href="{{ route('app.plan.show',$plan->id) }}" class="btn btn-success btn-sm"
                       style="    padding: 3px 6px;"><i class="fa fa-eye"></i></a>
                    <a href="{{ route('app.plan.edit',$plan->id) }}" class="btn btn-warning btn-sm"
                       style="    padding: 3px 6px;"><i class="fa fa-edit"></i></a>
                    @if(count($plan->companies)==0)
                    <a class="btn btn-danger btn-sm" data-mid="{{ $plan->id }}"
                       data-url="{!! URL::route('app.plan.destroy', $plan->id) !!}" data-toggle="modal"
