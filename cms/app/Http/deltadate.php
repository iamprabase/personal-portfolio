...
class Nepali_Calendar
{
    ...
    {
        if ($yy < 1440 || $yy > 2033) {
            return 'Supported only between 1940-2033';
        }
        if ($mm < 1 || $mm > 12) {
    ...
    public function eng_to_nep($yy, $mm, $dd)
    {
        ...
        if ($chk !== TRUE) {
            ...
            $this->_nep_date['month'] = $m <= 9 ? "0" . $m : $m;
            $this->_nep_date['date'] = $total_nDays <= 9 ? "0" . $total_nDays : $total_nDays;
...
function getDeltaDate($date)
{
    if ($date >= '2033-12-30') {
        return $date;
    } elseif ($date < '1440-01-01') {
        return $date;
    } else {
        $nepali_date = new Nepali_Calendar();
        ...
        if ($ncal == 1) {
            ...
            $date_ne = $nepali_date->eng_to_nep($year_en, $month_en, $day_en);

            if ($date_format == 'Y M d') {
                $final_date = $date_ne['year'] . ' ' . $date_ne['nmonth'] . ' ' . $date_ne['date'];
            } elseif ($date_format == 'd M Y') {
                $final_date = $date_ne['date'] . ' ' . $date_ne['nmonth'] . ' ' . $date_ne['year'];
            } elseif ($date_format == 'M d Y') {
                $final_date = $date_ne['nmonth'] . ' ' . $date_ne['date'] . ' ' . $date_ne['year'];
            } elseif ($date_format == 'Y-m-d') {
                $final_date = $date_ne['year'] . '-' . $date_ne['month'] . '-' . $date_ne['date'];
            } elseif ($date_format == 'd-m-Y') {
                $final_date = $date_ne['date'] . '-' . $date_ne['month'] . '-' . $date_ne['year'];
            } elseif ($date_format == 'm-d-Y') {
                $final_date = $date_ne['month'] . '-' . $date_ne['date'] . '-' . $date_ne['year'];
            } elseif ($date_format == 'Y/m/d') {
                $final_date = $date_ne['year'] . '/' . $date_ne['month'] . '/' . $date_ne['date'];
            } elseif ($date_format == 'd/m/Y') {
                $final_date = $date_ne['date'] . '/' . $date_ne['month'] . '/' . $date_ne['year'];
                ...
                $final_date = $date_ne['month'] . '/' . $date_ne['date'] . '/' . $date_ne['year'];
            } elseif ($date_format == 'D, M j Y') {
                $final_date = $date_ne['day'] . ', ' . $date_ne['nmonth'] . ' ' . $date_ne['date'] . ' ' . $date_ne['year'];
            } elseif ($date_format == 'D, j M Y') {
                $final_date = $date_ne['day'] . ', ' . $date_ne['date'] . ' ' . $date_ne['nmonth'] . ' ' . $date_ne['year'];
            } elseif ($date_format == 'D, Y M j') {
                $final_date = $date_ne['day'] . ', ' . $date_ne['year'] . ' ' . $date_ne['nmonth'] . ' ' . $date_ne['date'];
            } else {
                $final_date = $date_ne['date'] . ' ' . $date_ne['nmonth'] . ' ' . $date_ne['year'];
            }
            return $final_date;
            ...
            $date_format = config('settings.date_format');
            if (!$date_format) return Carbon::parse($date)->format(config('app.date_format'));
...
function getDeltaRange($date_range)
{
    $exploded = explode(' ', $date_range);
    if (count($exploded) == 3) {
        $first_date = getDeltaDate(date('Y-m-d', strtotime($exploded[0])));
        $second_date = getDeltaDate(date('Y-m-d', strtotime($exploded[2])));
...
function getDeltaDateForReport($date)
{
    if ($date >= '2033-12-30') {
        return $date;
    } elseif ($date < '1440-01-01') {
...
function getMthYrDeltaDate($date)
{
    if ($date >= '2076-01-01') {
        return $date;
    } else {
        ...
        if ($ncal == 1) {
            ...
            $date_ne = $nepali_date->eng_to_nep($year_en, $month_en, $day_en);
            return $date_ne['nmonth'] . ' ' . $date_ne['year'];
        } else {
...
function getDeltaDateFormat($date)
{
    if ($date >= '2033-12-30') {
        return $date;
    } elseif ($date < '1440-01-01') {
    ...
    if ($ncal == 1) {
        ...
        return $date_ne['year'] . '-' . $date_ne['month'] . '-' . $date_ne['date'];
    } else {
        return date("Y-m-d", strtotime($date));
    }
}

function getEngDeltaDate($date)
{
    $nepali_date = new Nepali_Calendar();
    $ncal = config('settings.ncal');
    if ($ncal == 1) {
        ...
        $month_en = $ndate[1];
        $day_en = $ndate[2];
        $date_ne = $nepali_date->nep_to_eng($year_en, $month_en, $day_en);
        return $date_ne['year'] . '-' . $date_ne['month'] . '-' . $date_ne['date'];
    } else {
        return date("d M Y", strtotime($date));
    }
...
function nepToEngConvert($date){
    $npdate = explode('-',$date);
    $npdate_obj = new Nepali_Calendar;
    $engdate = $npdate_obj->nep_to_eng($npdate[0],$npdate[1],$npdate[2]);
    if($engdate['month']>1 && $engdate['month']<10){
        $engdate['month'] = '0'.$engdate['month'];
    }
    $final_date = $engdate['year'] . '-' . $engdate['month'] . '-' . $engdate['date'];
    return $final_date;
}
...
function getDeltaDateForReport_mr($company_id,$date)
{
    if ($date >= '2033-12-30') {
        return $date;
    } elseif ($date < '1440-01-01') {
        ...
        $nepali_date = new Nepali_Calendar();
        $ncal = getCompanySettings($company_id)['ncal'];
        if ($ncal == 1) {
            $ndate = explode("-", $date);
            $year_en = $ndate[0];
            $month_en = $ndate[1];
