...
class AnalyticController extends Controller
{
    ...
    private function getLogicalDays($new_data,$type)
    {
        $arr = [];
        foreach ($new_data as $new) {
            foreach ($new as $k => $v) {
        ...
        $nepdates = [];
        $finalDates = [];
        if(isset($type) && $type=="nepali"){
            foreach($dates as $date){
                $exploded_date = explode('-',$date);
                array_push($nepdates,$this->neDate->eng_to_nep($exploded_date[0],$exploded_date[1],$exploded_date[2]));
            }
            $dates = $nepdates;
        }
        foreach($dates as $date){
            $exploded_date = explode('-',$date);
            $tempMD =$exploded_date[1].'-'.$exploded_date[2];
            array_push($finalDates,$tempMD);
        }
        $dates = $finalDates;
        $values = array_values($arr);
    ...
    private function getDates($id, $model, $type)
    {
        $date = [];
        $date_array = [];
        if ($model == "order") {
            $created_at = ($type == "employees") ? Order::employeeId($id)->pluck('order_date') :  Order::clientId($id)->pluck('order_date');
        } else {
            $created_at = ($type == "employees") ? Collection::employeeId($id)->pluck('payment_date') : Collection::clientId($id)->pluck('payment_date');
        }
        foreach ($created_at as $key => $value) {
            $parsed_date = Carbon::parse($value)->format('Y-m-d');
            array_push($date, $parsed_date);
        }
        $date_array = array_unique($date);
        $date_array_serialized = array_values($date_array);
        return $date_array_serialized;
    }
    ...
    public function countParameters(Request $request)
    {
        ...
        try {
            ...
            $parameters = array(
                ...
                'total_pending_collection_amount' => Collection::employeeId($request->id)->paymentDate($startDate, $endDate)->where('payment_method', "Cheque")->where('payment_status', "Pending")->sum('payment_received'),
                'total_cleared_collection_amount' => Collection::employeeId($request->id)->paymentDate($startDate, $endDate)->paymentStatus()->sum("payment_received"),
                'total_expense' => Expense::employeeId($request->id)->createdAt($startDate, $endDate." 23:59:59")->status()->sum('amount'),
    ...
    public function getNoOfOrders(Request $request)
    {
        ...
        try {
            ...
            if ($diffDays > 45) {
                $response = ($diff <= 11) ? $this->logicLayer($new_order_data_monthly,$request->type) : $this->logicLayer($new_order_data_yearly,$request->type);
            } else {
                $response = $this->getLogicalDays($new_order_data,$request->type);
            }
    ...
    public function getOrderValue(Request $request)
    {
        ...
        try {
            ...
            for ($i = 0; $i < count($dates); $i++) {
                ...
                array_push($custom_year_array, $year);
                $new_order_value_data_yearly[][$year] = (float) round($order, 2);
                $new_order_value_data_monthly[][$month] = (float) round($order, 2);
                $new_order_data[][$dates[$i]] = (float) round($order, 2);
            }
            $diffStartDate = Carbon::parse($dates[0]);
            $diffEndDate = Carbon::parse($dates[count($dates)-1]);
            $diff = $diffStartDate->diffInMonths($diffEndDate);
    ...
    {	
        ...
        try {
            ...
            $diffDays = $diffStartDate->diffInDays($diffEndDate);
            if ($diffDays > 45) {
                $response = ($diff <= 12) ? $this->logicLayer($new_collection_data_monthly) : $this->logicLayer($new_collection_data_yearly);
            } else {
                $response = $this->getLogicalDays($new_collection_data,$request->type);
            }
    ...
    {
        try {
            $currencySymbol = (config('settings.currency_symbol') == "Rs") ? "Rs." : config('settings.currency_symbol');
			if($request->user_type == "All User") {
                ...
                ->distinct('product_id')
                ->pluck('product_id');
				$totalOrders = Order::clientId($request->id)->orderDate($request->start_date, $request->end_date)->count();
				$totalOrderValues = Order::clientId($request->id)->orderDate($request->start_date, $request->end_date)->sum('grand_total');
				$totalPendingCollectionAmount = Collection::clientId($request->id)->paymentDate($request->start_date, $request->end_date)->where('payment_method', "Cheque")->where('payment_status', "Pending")->sum('payment_received');
				$totalClearedCollectionAmount = Collection::clientId($request->id)->paymentDate($request->start_date, $request->end_date)->paymentStatus()->sum("payment_received");
			} else {
				$lastVisited = Order::employeeId($request->e_id)->clientId($request->id)->latest()->first();
				$lastVisitedDate = ($lastVisited) ? $lastVisited->order_date : "No orders yet";
				$productsSold = Order::join('orderproducts', 'orders.id', '=', 'orderproducts.order_id')
					->where('orders.employee_id', '=', $request->e_id)
					->where('orders.client_id', '=', $request->id)
                    ->orderDate($request->start_date, $request->end_date)
                    ->distinct('product_id')
					->pluck('product_id');
				$totalOrders = Order::employeeId($request->e_id)->clientId($request->id)->orderDate($request->start_date, $request->end_date)->count();
				$totalOrderValues = Order::employeeId($request->e_id)->clientId($request->id)->orderDate($request->start_date, $request->end_date)->sum('grand_total');
				$totalPendingCollectionAmount = Collection::employeeId($request->e_id)->clientId($request->id)->paymentDate($request->start_date, $request->end_date)->where('payment_method', "Cheque")->where('payment_status', "Pending")->sum('payment_received');
				$totalClearedCollectionAmount = Collection::employeeId($request->e_id)->clientId($request->id)->paymentDate($request->start_date, $request->end_date)->paymentStatus()->sum("payment_received");
			}

            $parameters = array(
                'total_orders' => $totalOrders,
                'total_order_value' => $totalOrderValues,
                'total_pending_collection_amount' => $totalPendingCollectionAmount,
                'total_cleared_collection_amount' => $totalClearedCollectionAmount,
    ...
    public function getNoOfOrdersForParties(Request $request)
    {
        ...
        try {
			if($request->user_type == "All User") {
				...
				for ($i = 0; $i < count($dates); $i++) {
					$orders = Order::employeeId($request->e_id)->clientId($request->id)->where('order_date', $dates[$i])->count();
					$month = Carbon::parse($dates[$i])->format('M');
					$year = Carbon::parse($dates[$i])->format('Y');
					array_push($month_array, $month);
					array_push($custom_year_array, $year);
            ...
            $diffDays = $diffStartDate->diffInDays($diffEndDate);
            if ($diffDays > 45) {
                $response = ($diff <= 12) ? $this->logicLayer($new_order_data_monthly) : $this->logicLayer($new_order_data_yearly);
            } else {
                $response = $this->getLogicalDays($new_order_data,$request->type);
            }
    ...
    public function getOrderValueForParties(Request $request)
    {
        ...
        try {
			if($request->user_type == "All User") {
				...
				for ($i = 0; $i < count($dates); $i++) {
					$orders = (float) Order::employeeId($request->e_id)->clientId($request->id)->where('order_date', $dates[$i])->sum('grand_total');
					$month = Carbon::parse($dates[$i])->format('M');
					$year = Carbon::parse($dates[$i])->format('Y');
					array_push($month_array, $month);
					array_push($custom_year_array, $year);
    ...
    public function getCollectionAmountForParties(Request $request)
    {
        ...
        try {
            ...
            if($request->user_type == "All User") {
                ...
                for ($i = 0; $i < count($dates); $i++) {
                    $collection = (float) Collection::employeeId($request->e_id)->clientId($request->id)->whereDate('payment_date', $dates[$i])->PaymentStatus()->sum('payment_received');
                    $month = Carbon::parse($dates[$i])->format('M');
                    $year = Carbon::parse($dates[$i])->format('Y');
                    array_push($month_array, $month);
                    array_push($custom_year_array, $year);
